...
fileID = fopen('./results/devetalresult.txt','a');
fprintf(fileID,'sky \t thin-cloud \t thick-cloud \n');
...
for exp_number=1:number_of_exps
    
    disp (['Performing experiment number ', num2str(exp_number), ' out of ', num2str(number_of_exps)])
    ...
    for kot=1:testing_images
        
        index_number = index_number + 1;

        FileNames=file_list(testing_index(kot)).name;    
...
fprintf(fileID,'%f \t %f \t %f \n', sky_percent,thincloud_percent,thickcloud_percent);
...
disp (['sky error percentage: ',num2str(sky_percent)]);
disp (['thin cloud error percentage: ',num2str(thincloud_percent)]);
disp (['thick cloud error percentage: ',num2str(thickcloud_percent)]);
